...
class PromiseImpl<T = never> {
    ...
    public static race(promises: Array<PromiseImpl<any>>): PromiseImpl<any> {
        ...
        if (promises.length === 0) {
            ...
            for (const promise of promises) {
                promise.then(value => p.resolve(value), reason => p.reject(reason));
            }
        }
        return p;
    }
    ...
    public constructor();
    public constructor(executor: (onfulfilled: () => void, onrejected: (reason?: any) => void) => void);
    public constructor(executor: (onfulfilled: (value: T) => void, onrejected: (reason?: any) => void) => void);
    public constructor(executor?: (onfulfilled: (value?: T) => void, onrejected: (reason?: any) => void) => void) {
        if (executor) {
            try {
                executor(
    ...
    protected reject(reason: any): void {
        ...
        this.queue.forEach(d => {
            if (typeof d.onRejected !== "function") {
                // if reject isnt a function -> ignore it
    ...
    protected fulfill(value: T): PromiseImpl<T> {
        ...
        this.queue.forEach(d => {
            if (typeof d.onFulfilled !== "function") {
                // if fulfill isnt a function -> ignore it
